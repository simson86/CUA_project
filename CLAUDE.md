...
- ✅ **접근성 버튼 트리거 — 앱을 안 열고 작업 맡기기** — 어느 앱에서든 시스템 접근성 버튼을 누르면 투명 카드(`VoiceTriggerActivity`)가 뜨고, `VoiceInput` 으로 듣고, 3초 카운트다운(취소 가능) 뒤 **카드를 닫고 600ms 기다렸다가**(`TRIGGER_SETTLE_MS`, 안 기다리면 첫 스크린샷에 카드가 찍힌다) 서비스가 실행한다. 첫 화면은 그때 보던 앱이다. 설정은 `RunConfig` 가 저장값을 읽고, 기록은 `RunHistory` 로 앱 UI 실행과 같은 `run_history.txt` 에 `[경로] 접근성 버튼` 표시와 함께 남는다. `runTask` 를 거치므로 `attended = true`(사람이 누른 실행). **실행 중에 누르면** 새 지시 대신 `[중단]`/`[닫기]` 카드 — `requestCancel()` 이라 현재 단계가 끝나야 멈추고, 카드가 스크린샷에 찍히므로 8초 뒤 스스로 닫힌다. **액티비티를 거치는 이유는 마이크** — 접근성 서비스는 포그라운드가 아니라 마이크를 못 연다. `runTask` 에 **중복 실행 가드**(`isRunning`, `compareAndSet`)를 새로 넣었다 — 카드의 카운트다운 사이에 앱에서 실행을 누를 수 있어서다. ⚠️ **소켓 `RUN` 은 이 가드 밖이고 `runTask` 로 합치면 안 된다**(무인 경로가 `attended=true` 로 바뀌어 PR #15 가 고친 3분 대기 버그가 되살아난다). Google Assistant(2026-09-04 종료 중)·Gemini(제3자 앱에 작업을 넘길 공개 경로 없음)는 조사 후 버렸다. 근거·버린 대안: `docs/reference/android_run-handsfree-impl-2026-09-15.md`. **빌드 확인, 실기기 미검증**
...
### 백그라운드에서 방치된 앱은 **얼어붙는다** — 소켓이 죽은 것처럼 보인다 ★
소켓 명령이 갑자기 타임아웃나기 시작하면 **앱이 얼었는지부터 의심할 것.** 겉보기 증상이
'서버가 죽었다'와 완전히 똑같아서 한참 헤맸다(2026-09-16, 네트워크·adb·서버 코드를 차례로
의심했다):

| 관측 | |
|---|---|
| 프로세스 | 살아 있음 (pid 유지) |
| 포트 8080 | `LISTEN` (커널이 잡고 있으니 당연) |
| accept 큐 | 비어 있음 |
| 스레드 | 전부 `S`, CPU 0% |
| **연결** | **폰이 자기 자신(127.0.0.1)에 붙어도 타임아웃** |

마지막 줄이 결정적이다 — **루프백도 안 되면 네트워크 문제가 아니다.**

판별은 1분이면 된다:
```
백그라운드 방치 → 타임아웃 / 앱을 앞으로 → OK / 다시 백그라운드 → OK
```
앞으로 가져왔을 때 살아나면 얼어 있던 것이다.

**해제**(측정 전에 반드시):
```bash
adb shell dumpsys deviceidle whitelist +com.cua.a11
```
검증: 20초·65초·155초 방치 후에도 전부 응답(그 전에는 1분쯤 뒤부터 타임아웃).
사용자 설정으로는 `설정 > 배터리 > 앱 절전 관리`에서 제외. **접근성 서비스라고 안전하지
않다** — 프로세스가 죽지 않는 것과 얼지 않는 것은 다른 이야기다.

⚠️ 재부팅하면 whitelist 가 풀린다. 측정 배치 전에 다시 넣을 것.

### 소켓 서버는 연결마다 스레드를 띄운다 ★
예전엔 `accept()` 루프에서 명령 처리까지 직접 했다. 그러면 **한 군데라도 안 끝나면 서버가
통째로 죽는다.** 실제로 세 번 겪었고 매번 물린 자리가 달랐다:

| | 물린 곳 | 고침 |
|---|---|---|
| ① | 인계 카드가 없는 사람을 3분 기다림 | `attended` 깃발 |
| ② | `cuCall` 이 5분 넘게 안 돌아옴 | `callTimeout(120)` — `readTimeout` 은 **한 번의 read** 기준이라 요청 전체엔 상한이 없다 |
| ③ | 응답을 쓰다 멈춤 | **구조** — 자바 소켓엔 쓰기 타임아웃이 없어 코드로는 못 막는다 |

③에서 '하나씩 막기'는 끝이 없다는 게 분명해져 구조를 바꿨다. 이제 물려도 **그 스레드만**
죽는다. 대신 **동시 실행 방지가 필요해졌다** — `agentBusy` 깃발을 앱 UI·소켓이 공유한다.
두 에이전트가 같은 화면을 조작하면 화면도 기록도 엉킨다.

⚠️ `agentBusy` 가 `true` 로 새면 **이후 모든 실행이 영구히 막힌다.** 반드시 `finally` 로 풀 것.
...
- **Auto-commit:** a `Stop` hook in `.claude/settings.local.json` runs `git add -A` + commits (message `[auto] <timestamp>`) at the end of each turn, only when something changed. **On `main` it does nothing** (the hook exits early) — auto-commits only land on feature branches. Make your own descriptive commits for real work; the hook is a safety net. ⚠️ `.claude/settings.local.json` is **gitignored**, so this hook does **not** survive a clone or a repo move — it was silently lost in the 2026-08-04 move and only noticed on 2026-09-02. Recreate it after a fresh clone, same as `.git/hooks/post-commit` below.
