...
⚠️ **`3.7`·`3.8` 은 "API 가 받아준다"까지만 확인됐다.** 실기기 멀티턴 완주·턴 수·정확도는
안 쟀다. 이 프로젝트에서 속도는 한 턴의 토큰이 아니라 **총 턴 수**이므로(벽시계의 ~85%가 API
왕복), 기본 모델을 올리려면 폰에서 같은 과제로 A/B 를 재야 한다. `DEFAULT_MODEL` 은 그때까지
`gemini-3.5-flash` 로 둔다. 참고로 1턴짜리 `total_thought_tokens` 숫자는 표본이 1이라 순서도
