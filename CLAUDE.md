...
- ✅ **자격증명 입력 인계** — 에이전트가 값을 모르는 입력칸을 사용자에게 넘김. 카드는 `[입력했어요]`/`[중단]` 2종이고 값은 사용자가 뒤 앱의 진짜 키보드로 직접 침(앱이 값을 쥐지 않음 → 로그에도 안 남음). **감지 경로가 둘**:
...
### 커스텀 함수는 쓸 수 있다 — 역시 평면. 그리고 선언만으론 안 불린다 ★
...
⚠️ **선언만 하면 모델은 한 번도 안 부른다.** 시스템 프롬프트의 "추측하지 말고 이 함수를 불러라" 규칙과 **한 쌍**이다. 같은 화면·같은 요청 반복 실측(인증번호 화면, 3.5-flash/low):

| 조건 | `request_user_input` 호출 |
|---|---|
| 도구만 선언, 규칙 없음 | **0/5** |
| 도구 + 프롬프트 규칙 | **10/15** |

100%가 아닌 건 `safety_decision`과 같은 비결정성이다. 다만 **빗나간 5회 중 가짜 값을 입력한 경우는 0회**였고 전부 `list_apps`(무해한 턴 낭비)로 샜다 — 실패가 안전한 쪽으로 난다. 그래서 위험이 가장 큰 비밀번호만 코드가 확정적으로 막고(위 자격증명 게이트), 나머지는 이 경로에 맡긴다.
