...
**호출률 실측**(네이버 로그인 화면, 3.5-flash/low, N=5):

| 조건 | `request_user_input` |
|---|---|
| 로그인이 필요한 작업("네이버에 로그인해줘"), 규칙 있음 | 5/5 |
| 같은 작업, 규칙 없음 | 5/5 |
| **로그인이 필요 없는 작업**("회원가입 링크 눌러줘") | **0/5** (전부 `click`) |

세 번째 줄이 핵심이다 — **화면에 아이디·비밀번호 칸이 있어도 그 작업에 필요 없으면 안 부른다.** 트리거가 '화면 생김새'가 아니라 '내가 막혔는가'라서 그렇다.
