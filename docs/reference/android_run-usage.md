...
> 로컬 전용(.gitignore `hybrid-*`). ①번(폰 단독) 앱을 **실제로 돌리는 방법**만 모음.
> 코드 설명은 각 step 문서(step5=단발 호출, step6=자율 루프)·안전은 `hybrid-todo-safety-confirm.md`.
...
- 근본 해결=`hybrid-todo-safety-confirm.md`(사람 확인 다이얼로그로 교체) 후 진행.
