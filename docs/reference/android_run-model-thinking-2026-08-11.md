...
>
> ⚠️ **2026-09-08 갱신 — 이 문서의 한 가지 전제가 깨졌다.** 여기서는 사고수준을 "값이 4종으로
> 고정"이라 보고 모델 × 사고수준을 **자유 조합**으로 설계했다. 그런데 `gemini-3.7-flash`·
> `gemini-3.8-flash` 를 추가하며 재보니 **두 모델은 `minimal` 을 400 으로 거절한다**
> (`Allowed values are: medium, low, high`). 그래서 사고수준 목록의 출처가
> `CuClient.THINKING`(전체) → `CuClient.thinkingFor(model)` 로 바뀌었고, 모델을 바꾸면
> 사고수준 드롭다운이 다시 그려진다. 아래 §1·§3·§4 의 코드 스니펫은 그 이전 모습이다 —
> 구조(값이 흐르는 길·우선순위)는 그대로 유효하지만 **목록이 고정이라는 부분만 낡았다.**
> 실측표는 CLAUDE.md Gotchas "사고수준은 모델마다 받는 값이 다르다" 참조.
