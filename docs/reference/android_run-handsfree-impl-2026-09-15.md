...
**2026-09-15 작성.** 브랜치 `handsfree`. 설계 가이드는 `android_run-handsfree-2026-09-02.md`,
...
5. 진행은 기존 진행 표시 띠·알림으로 보이고, 기록은 `run_history.txt` 에 `[경로] 접근성 버튼`
   표시와 함께 남는다
...
| 에이전트가 접근성 버튼을 누를 수 있음 | 제스처 내비의 떠 있는 버튼은 좌표가 겹칠 수 있다. 누르면 카드가 뜨는데, 가드 때문에 "이미 실행 중" 으로 닫힌다 — 실행이 겹치지는 않지만 한 턴을 헛쓴다. 기존 🔶 자체 안전 게이트 항목과 같은 뿌리 |
...
| 9 | 트리거 실행 중에 버튼 다시 탭 | 카드가 "이미 실행 중"을 보여 주고 닫힘 | — |
