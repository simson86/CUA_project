...
#### 음성으로 목표 말하기
목표 입력칸 옆 **마이크 버튼**을 누르면 듣기 시작(아이콘이 빨갛게). 말하는 중 인식 결과가
입력칸에 실시간으로 찍히고, **한 번 더 누르면** 확정된다. 이후 **3초 카운트다운** 뒤 자동
실행되며 그 사이 `취소`를 누르면 멈춘다. 직접 타이핑하는 기존 방식도 그대로 쓸 수 있다.

- 첫 사용 시 **마이크 권한**을 묻는다. 거부해도 앱은 동작한다(직접 입력).
- 인식 엔진은 **폰에 설정된 기본 음성인식**(설정 > 기본 앱 > 디지털 어시스턴트)을 쓴다.
  기기·계정에 따라 인식률이 다르다.
- 인식이 좀 틀려도 문장은 Gemini에게 가므로 웬만한 오탈자는 모델이 흡수한다.
