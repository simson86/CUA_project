...
| **6** | **reconciliation + 승격** | ⬜ **다음 ← 순서를 앞당김.** 틀린 기억이 해롭다는 게 2차에서 확인됐다 |
...
**Unit 4 재측정 — (가)+(나) 묶음.** 판단 ①이 음성이었지만 세 과제 모두 다른 이유로
오염돼, 지금 결과만으로 리플렉터를 접기엔 근거가 약하다.

1. **주입 버그 수정** — `MemoryEntity.scope`(`pkg|vendor|system`)를 실제로 구현해 기억이
   패키지를 넘어 따라가게 한다. 어차피 버그다 — 지금은 칸만 있고 `pkg` 말고 쓰는 코드가 없다.
2. **과제 다시 고르기** — **탐색형 낭비가 20% 이상**인 것으로. 읽기형·대기형은 기억이 못 줄인다.
3. **데워질 때까지 버리는 실행을 먼저** 돌리고 **안정된 행동을 보고** 기억을 쓴다.
   이번엔 가장 차가운 1~2회차를 보고 써서 데워진 3~12회차에 대고 쟀다.
4. **행동으로 옮길 수 있는 문장** — "순서는 A→B→C" 가 아니라 "X 를 하면 Y 에 닿는다".

storage 가 다음 실험의 최적 후보다 — 낭비가 탐색형이고, 데워진 뒤에도 7.4턴이 남았고,
주입 버그를 고치면 그 자체로 깨끗한 재시험이 된다.

**Unit 5(리플렉터)는 그 결과가 나올 때까지 보류.**
