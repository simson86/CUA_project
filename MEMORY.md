...
| 2 | `memory` 테이블 + 읽기 경로 | ⬜ **다음** |
| 3 | 기억 목록 UI (보기·추가·수정·삭제) | ⬜ |
...
**Unit 2 — `memory` 테이블 + 읽기 경로.** Room 스키마 버전을 2 로 올려 `memory` 를 넣고,
`MemoryGateway` 를 세우고, `a11service.appNote()`·`CuClient.taskNote()` 의 하드코딩 맵을
DB 조회로 바꾼다. **여기서 처음으로 기억이 실제 주입된다.**

랭킹은 만들지 않는다 — 기억이 몇 개일 때 점수식은 무의미하다.
`WHERE kind=? AND pkg=? AND state='ACTIVE' LIMIT 2` 면 충분하고, 점수식은 Unit 7 이다.
