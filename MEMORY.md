...
| **4** | **측정 세트 + 소켓 `DUMP`** | 🔶 **진행 중** → **판단 ①** (아래 계획) |
| 5 | 리플렉터 (추출 + §3 필터) | ⬜ |
| 6 | reconciliation + 승격 | ⬜ → **판단 ②** (아래) |
| 7 | 랭킹 점수식 | ⬜ |
...
| **판단 ②** | 에이전트가 스스로 **쌓은** 기억이 시간이 갈수록 도움이 되나 | 종단(longitudinal) | Unit 6 |
...
3. **정확도는 안 잰다.** 지표가 턴 수라 모델이 가격을 틀리게 말해도 `success` 다 —
