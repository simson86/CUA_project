...
### 그래서 신뢰할 수 있는 결론
**musinsa 가 가장 깨끗하다** — 기억이 96턴 중 85턴에 붙었고, 여유도 남아 있었고, 결과는
**정확히 ±0.0%** 였다. 스크롤 횟수도 A 13 : B 13 으로 같다. 상품 페이지 배치를 알려줘도
**모델은 똑같이 오르내렸다.**

> **앱 UI 지도 급 기억은 이 세 과제에서 턴을 줄이지 못했다.**
> 특히 제대로 주입된 경우(musinsa)에 효과가 **정확히 0** 이었다.

### 이것이 말해주지 *않는* 것
- "기억 시스템이 쓸모없다" ← 잰 것은 **1급(앱 UI 지도)** 하나뿐이다
- "리플렉터가 실패한다" ← 리플렉터는 존재하지도 않는다
- melon·storage 결과 ← 각각 바닥 효과와 주입 버그로 오염됐다
