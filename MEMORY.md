...
### 소켓 경로는 신뢰할 수 없다 — 원인 미상 ★
PC 에서 소켓 `RUN` 으로 돌리면 **턴 2 에서 멈춘다**(2026-09-13, 2/2 재현). 같은 작업을
**앱 UI 로 돌리면 완주한다.**

```
02:37:41  턴 2 open_app
02:44:03  A11y: client error: timeout        ← 6분 22초 뒤
```

⚠️ **커밋 `a89f9ed` 은 이걸 "화면이 꺼져 Doze 에 들어갔다"고 단정했는데 철회한다.**
근거가 없었고, 숫자도 안 맞는다 — `readTimeout` 이 60초인데 왜 6분 22초인지 설명하지
못한다. `HANDOVER_TIMEOUT_MIN = 3L` 이 걸린 `handoverWait` 의 `while(true)` 루프(3분 × 2)가
시간상 훨씬 잘 맞지만 **그쪽도 확증이 아니다**(`handoverWait` 는 로그를 안 남긴다).

**화면을 켜고 한 재시험은 무효였다.** 명령이 앱에 닿지도 않았다:

| 관측 | |
|---|---|
| 포트 8080 리스닝 | ✅ `st=0A`, uid 10602 |
| 소켓 스레드 `Thread-2` | 살아 있음, `S`(블록) |
| 268초 동안 앱 로그 | **한 줄도 없음** — `client connected` 조차 |

TCP 는 커널이 받았지만 앱이 그 연결을 꺼내지 않았다 = 단일 스레드 소켓 서버가
`accept()` 가 아닌 곳에 물려 있었다. 그래서 "화면 상태가 원인인가"는 **아직 안 갈렸다.**

**지금 아는 것은 이것뿐이다: 소켓 경로는 믿을 수 없고 이유를 모른다.**
주력인 앱 UI 는 멀쩡하므로 당장 막히지는 않는다. 다만 **Unit 4(측정 자동화)가
`tools/bench_*.py` → 소켓으로 돌아가므로, Unit 4 를 시작하기 전에 반드시 해결해야 한다.**
첫 수는 앱 재시작으로 소켓 서버를 새로 띄우고 `handoverWait` 에 로그를 넣는 것.

부수 확인: 그 실패들이 `run.outcome='error'`, `turnsUsed=0` 으로 남았다 — **설계대로다.**
outcome 초기값을 `error` 로 두고 finally 에서 기록한 것이 예외 경로를 정확히 잡았다.
...
⚠️ **시작하기 전에 소켓 경로부터 고쳐야 한다** — `tools/bench_*.py` 가 소켓으로 돌리는데
지금 그 길이 신뢰할 수 없다(위 Gotchas). 안 고치고 재면 측정값이 오염된다.

측정 스크립트가 **반드시 해야 할 두 가지**(안 하면 숫자를 믿을 수 없다):
1. `memoryReadFailed = 1` 인 실행을 결과에서 뺀다 (`MemoryDao.contaminatedRuns()`)
2. **기대 건수와 실제 `run` 행 수를 대조**한다 — DB 가 통째로 죽으면 행 자체가 없다
